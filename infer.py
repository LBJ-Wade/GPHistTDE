...
def main():
    ...
    parser.add_argument('--rsdrag', type = float, default = 147.09,# from Planck 2018 full CMB
    ...
    SN_data = np.loadtxt('lcparam_DS17f.txt',skiprows=1)
    z_SN = SN_data[:,1]
    mu_SN = SN_data[:,4]+19.34911682
    SN_cov_load = np.loadtxt('sys_DS17f.txt')
    cov_mu = SN_cov_load[1:].reshape(40,40) + np.diag(SN_data[:,5]**2)
    ...
    loaded = np.loadtxt('BAO_consensus_results_dM_Hz_nostrings.txt')
    dist_BOSS = loaded[:,1]
    i_reorder = [0,2,4,1,3,5]
    dist_BOSS = dist_BOSS[i_reorder] #reodering to match how the theory GP part is ordered
    z_BOSS = loaded[::2,0]
    cov_BOSS = np.loadtxt('BAO_consensus_covtot_dM_Hz.txt')
    cov_BOSS = cov_BOSS[i_reorder,:][:,i_reorder]
    ...
    posteriors = [
        ...
        # Local H0 measurement from Reis 2011 (dx.doi.org/10.1088/0004-637X/730/2/119)
        #gphist.posterior.LocalH0Posterior('H0',73.24,1.74),
        gphist.posterior.LocalH0Posterior('H0',73.24,0.7324),


        # BOSS Lya-Lya & QSO-Lya from Delubac 2014. 1404.1801
        #gphist.posterior.BAOPosterior('Lya',2.34,9.15,0.20,36.46,1.22,-0.38,args.rsdrag),
        #from Bautista 2017 1702.00176 Lya-Lya
        #gphist.posterior.BAOPosterior('Lya',2.33,9.07,0.31,37.77,2.13,-0.38,args.rsdrag),
        #from arxiv 1708.02225 Lya-QSO
        #gphist.posterior.BAOPosterior('Lya',2.40,9.01,0.36,35.7,1.7,-0.38,args.rsdrag),
        # from arxiv 1708.02225 Lya-Lya & Lya-QSO
        gphist.posterior.BAOPosterior('Lya',2.40,8.94,0.22,36.6,1.2,-0.38,args.rsdrag),
        # debugging
    ...
    # The choice of CMB posterior depends on whether we are allowing for high-z modifications
    if args.Neff:
        print('LCDM+Neff:TT')
        #posteriors.append(
        ...
        #posteriors.append(
        #    gphist.posterior.CMBPosterior('CMB',args.zstar,1.9273724E-01,1.2749623E+01,
        #    1.68889856E-06 / 2,5.894375E-05 / 2,2.2449463E-03 / 2))



    SN_post = gphist.posterior.SNPosterior('SN',z_SN,mu_SN,cov_mu)
    BOSS2018post = gphist.posterior.BOSS2018Posterior('LRG', z_BOSS, dist_BOSS ,cov_BOSS,args.rsdrag)
    ...
    posterior_redshifts = np.concatenate((posterior_redshifts,BOSS2018post.zpost))

    posterior_names = np.concatenate((posterior_names,np.array([SN_post.name])))
    posterior_names = np.concatenate((posterior_names,np.array([BOSS2018post.name])))

    posteriors.append(SN_post)
    posteriors.append(BOSS2018post)
    ...
    for hyper_offset in range(args.hyper_count):
        ...
        combined_DH_hist,combined_DA_hist,combined_de_hist,combined_phi_hist,combined_f_hist,combined_q_hist,combined_w_hist = None,None,None,None,None,None,None
        ...
        while samples_remaining > 0:
            ...
            posteriors_nlp = gphist.analysis.calculate_posteriors_nlp(
                evol.zvalues,DH,DA,mu,posteriors)
            ...
            DH_re_ds,DA_re_ds = DH_realizations[:,:,i_ds],DA_realizations[:,:,i_ds]
            if args.growth:
                print('calculating growth functions...')
                phi0_ds,f0_ds = evol.get_phi_take3(DH0_ds[np.newaxis,:],evol.svalues[i_ds])
                phi_ds,f_ds = evol.get_phi_take3(DH_ds,evol.svalues[i_ds])
            ...
            w0_evol = evol.get_w_deriv(de0_evol[0,np.newaxis,:],evol.svalues[i_ds])
            #print args.dark_energy
            if args.dark_energy:
                de_evol_upsample = gphist.cosmology.get_dark_energy_evolution(evol.zvalues,DH)
                de_evol = de_evol_upsample[:,:,i_ds]
                w = evol.get_w_deriv(de_evol_upsample[0],evol.zvalues)
                w_ds = np.nan_to_num(w[:,i_ds])
            else:
                w_ds = None
                de_evol = None

            print('done calculating DE eos')


            # Build histograms for each downsampled redshift slice and for
            # all permutations of posteriors.
            DH_hist,DA_hist,f_hist,phi_hist,de_hist,w_hist,q_hist = gphist.analysis.calculate_histograms(
                DH_ds,DH0_ds,DA_ds,DA0_ds,f_ds,f0_ds,phi_ds,phi0_ds,de_evol,de0_evol,w_ds,q_ds,q0_ds,posteriors_nlp,
        ...
        if args.output:
            ...
            np.savez(output_name,
                ...
                f_hist=combined_f_hist,de_hist=combined_de_hist,w_hist=combined_w_hist,q_hist=combined_q_hist,
                DH0=DH0_ds,DA0=DA0_ds,phi0=phi0_ds,f0=f0_ds,de0=de0_evol,w0=w0_evol,q0=q0_ds,
                fixed_options=fixed_options,variable_options=variable_options,
                bin_range=bin_range,hyper_range=hyper_range,
                DH_realizations=DH_re_ds,DA_realizations=DA_re_ds,
