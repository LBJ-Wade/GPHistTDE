...
def main():
    ...
    for iperm,perm in enumerate(perms):
        ...
        if args.full:
            ...
            if args.examples:
                print(DH_realizations.shape[1])
                for i in range(DH_realizations.shape[1]):
                    plt.plot(1+zvalues,DH_realizations[iperm,i,:]/DH0)
        ...
        if args.dark_energy:
            ...
            for ide in range(len(de_hist)):
                ...
                #plt.plot(example_z,example_de,'r')
                #plt.plot(example_z,example_de_3,'m')
        ...
        if args.dark_energy:
            de_limits = gphist.analysis.calculate_confidence_limits(
                    de_hist[0,iperm,:iend],[args.level],de_bin_range)
            # The w_hist arrays are histograms of w/(-1) so there is a -1 infront of the limits to plot w
            w_limits = gphist.analysis.calculate_confidence_limits(w_hist[iperm,:],[args.level],[0.01,4.01])
            plt.fill_between(zvalues[:iend],w_limits[0,:iend]-3.01,w_limits[-1,:iend]-3.01,
                        facecolor='blue',alpha=0.25)
            plt.plot(zvalues[:iend],w_limits[0,:iend]-3.01,'b:')
            plt.plot(zvalues[:iend],w_limits[1,:iend]-3.01,'b-')
            plt.plot(zvalues[:iend],w_limits[2,:iend]-3.01,'b:')
            #plt.plot(zvalues[:iend],w0[0,:iend]-3.01,'k--')
            #plt.plot(zvalues[:iend],w_median[:iend],'k')
            #plt.plot(example_z,example_w,'r')
            #plt.plot(example_z,example_w_3,'m')
            #plt.plot(zvalues[:iend],w0[0,:iend],'g--')
            plt.xlabel(r'$z$')
            plt.ylabel(r'$w(z)$')
            plt.xlim(0,3)
            #plt.ylim(-2.5,-0.5)
            plt.savefig(args.output + '_EoS-' + name + '.' + args.plot_format)
            plt.clf()
    ...
    # Plot growth function phi.
        if args.growth:
            ...
            phi_ratio_limits = gphist.analysis.calculate_confidence_limits(
                                                             phi_hist[iperm],[args.level],bin_range)
            ...
            plt.grid(True)
            plt.fill_between(zvalues,f_limits[0,::-1],f_limits[-1,::-1],facecolor='blue',alpha=0.25)
            plt.plot(zvalues,f_limits[0,::-1],'b:')
            plt.plot(zvalues,f_limits[1,::-1],'b-',label = 'Median')
            plt.plot(zvalues,f_limits[2,::-1],'b:')
            plt.plot(zvalues,f0[0,::-1],'k',label = r'$\Lambda CDM$')
            plt.plot(zvalues,omega_matter[::-1]**gamma_best_fit,'c',label = r'$\Omega_m^\gamma , \gamma = $ ' +str(gamma_best_fit))
            plt.plot(zvalues,omega_matter0[::-1]**0.55,'g',label=r'$\Omega_{m, \Lambda CDM}^{0.55}$')
            #plt.plot(example_z, example_f,'r',label='My tanh')
            #plt.plot(example_z, example_f_3,'m',label='My w0wa')
            #plt.plot(CLASS_z,CLASS_tanh,'y',label='CLASS tanh')
            #plt.plot(CLASS_z,CLASS_w0wa,'orange',label='CLASS LCDM')
            #plt.plot(lna,phi0*phi_realizations[iperm,0], label ='Realization 1')
            #plt.plot(lna,phi0*phi_realizations[iperm,1], label ='Realization 2')
            plt.xlim(0,3)
            plt.ylim(0,1.2)
            ...
            plt.grid(True)
            plt.fill_between(zvalues,phi_limits[0,::-1],phi_limits[-1,::-1],facecolor='blue',alpha=0.25)
            plt.plot(zvalues,phi_limits[0,::-1],'b:')
            plt.plot(zvalues,phi_limits[1,::-1],'b-',label = 'Median')
            plt.plot(zvalues,phi_limits[2,::-1],'b:')
            plt.plot(zvalues,phi0[0,::-1],'k',label = r'$\Lambda CDM$')
            plt.xlim(0,3)
            plt.xlabel(r'$z$')
            plt.ylabel(r'$\phi(z)$')
            plt.legend(loc='best')
            plt.savefig(args.output + '_growth_phi-' + name + '.' + args.plot_format)
            plt.close()
