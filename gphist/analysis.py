...
def calculate_posteriors_nlp(zprior,DH,DA,mu,posteriors):
	...
	Args:
        ...
        mu(ndarray): Array of shape (nsamples,nz) of mu(z) values to use.
...
def calculate_histograms(DH,DH0,DA,DA0,f,f0,phi,phi0,de_evol,de0_evol,w,q,q0,nlp,num_bins,min_value,max_value):
	...
	w_hist = np.empty((nperm,nz-1,num_bins+2)) if de_evol is not None else None
	...
	if de_evol is not None:
        ...
        #REMEMBER w_hist is the histogram of 1+w
		w_bin_indices = get_bin_indices(w.T,num_bins,min_value=0.01,max_value=4.01)
	...
	for iperm,perm in enumerate(perms):
		...
		for ihist in range(nz):
			...
			if de_evol is not None and ihist < nz-1:
			    w_hist[iperm,ihist] = np.bincount(
				    w_bin_indices[ihist],weights=perm_weights,minlength=num_bins+2)
	...
	return DH_hist,DA_hist,f_hist,phi_hist,de_hist,w_hist,q_hist
