...
class LogScale(object):
    ...
    def get_phi_take3(self,DH,svalues):
    #returns the phi and the growth function f = 1 + phi_dot/phi
        ...
        """
        #2nd order ODE is phi'' + (4+H'/H)phi' + (3+2H'/H)phi=0
        #coupled 1st order ODE is phi'=phi_dot and phi_dot' =-(4 + H'/H(s))*phi_dot - (3 + 2*H'/H(s))*phi
        ...
        H_z = 1./DH[:,::-1]
        H_prime = np.gradient(H_z,lna,axis=1)
        HpoH = H_prime/H_z
        for j in range(len(lna[istart:])-1):
            phi[:,istart+j+1] = phi[:,istart+j] + dlna[istart+j]*phi_dot[:,istart+j]
            phi_dot[:,istart+j+1] = phi_dot[:,istart+j] + dlna[istart+j]*(-(4 + HpoH[:,istart+j])*phi_dot[:,istart+j] - (3 + 2*HpoH[:,istart+j])*phi[:,istart+j])
    ...
    #returns the deceleration parameter q+1 = - h_dot/h^2 or dh/da *a/h
        ...
        a = np.exp(lna)
        H_z = 1./DH[:,::-1]
        H_prime = np.gradient(H_z,a,axis=1)
        return -H_prime*a/H_z


    def get_w_deriv(self,DE,zvalues):
        ...
        Returns:
            ndarray:2D array of tabulated w(z) with the same
                ...
                zvalues[j].
        """
        # w(z) as -1-1/3 dln(rho)/dlna
        #return w+3.01 so nans, which are reset as 0 get sent to underflow bin since we bin from -3 to 1
        #lna = -zvalues[::-1]*np.log(1 + self.zmax)
        #dlna = np.gradient(lna)
        lnrho = np.log(DE)
        lnrho_prime = -np.gradient(lnrho,np.log(1+zvalues),axis=1)
        return 2.01-(1./3.)*lnrho_prime
